...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. 구글 제미나이 API 호출 (v1beta 사용으로 2.0 및 1.5 모델 동시 지원)
    ...
    // 4. 할당량 초과(429) 시 1.5-flash-8b로 자동 폴백 (더 높은 할당량)
    if (response.status === 429 && targetModel !== 'gemini-1.5-flash-8b') {
      console.warn(`Quota exceeded for ${targetModel}. Falling back to gemini-1.5-flash-8b...`);
